...
var division_sacs = ["HNC7", "HNCC", "HNCD", "HNC9", "HNC4", "HNCB", "HNC5", "HNC1"];
//var division_sacs = ["HNC7", "HNCC", "HNC9"];
//var division_sacs = ["HNC7"];
...
var division = ['CCR - Center for Cancer Research', 'DCB - Division of Cancer Biology', 'DCCPS - Division of Cancer Control and Population Sciences', 'DCEG - Division of Cancer Epidemiology and Genetics', 'DCP - Division of Cancer Prevention', 'DCTD - Division of Cancer Treatment and Diagnosis', 'DEA - Division of Extramural Activities', 'OD - Office of the Director'];
//console.log("\n *STARTING* \n");
...
for (i=0; i < division_sacs.length; i++) {
	...
	console.log(division_sacs[i] +" : "+division[i]);
	...
	printBranchesDrupalSorted(branches);
...
function printBranchesDrupalSorted(branches) {
	//console.log("Branches/LABS");

	branches.sort(function(a, b){ // sort object by Employee Name
	...
	for(j=0; j < branches.length; j++) {
		...
		//console.dir(branches[j].subbranches);
		var mysub = branches[j].subbranches.slice();

		mysub.sort(function(a, b){ // sort object by Employee Name
			var nameA=a.path.toLowerCase(), nameB=b.path.toLowerCase()
			if (nameA < nameB) //sort string ascending
		 		return -1 
			if (nameA > nameB)
		...
		//console.log("LENGTH: "+branches[j].subbranches.length);

		for(k=0; k < mysub.length; k++) {

		  //	console.log(k);
		  if(mysub[k].name.search("Section")==0) {
		  	console.log('            "'+ mysub[k].sac + '" : "'+ branches[j].shortName + ' ' + mysub[k].shortName +' - ' +titleCase(mysub[k].name) +'"');
		  }
		}


		//for(k=0; k < branches[j].subbranches.length; k++) {
		//	console.log('            "'+ branches[j].subbranches[k].sac + '" : "'+ branches[j].shortName + ' ' + branches[j].subbranches[k].shortName +' - ' +titleCase(branches[j].subbranches[k].name) +'"');
		//}
...
function titleCase(str) {
	...
	for (var i = 0; i < str.length; i++) {
	  //Don't capitalize
	  if((str[i] == 'of' || str[i] == 'the' || str[i] == 'and' || str[i] == 'to' || str[i] == 'for') && i>0) {
	    str[i] = str[i].charAt(0).toLowerCase() + str[i].slice(1); 
	  } else {
	    str[i] = str[i].charAt(0).toUpperCase() + str[i].slice(1); 
	  }
	  //Capitalize ALL
	  if(str[i] == 'Dna' || str[i] == 'Rna' || str[i] == 'Hiv' || str[i] == 'Aids' || str[i] == '(sbir)' || str[i] == 'Nci' || str[i] == 'It') {
	    str[i] = str[i].toUpperCase(); 
//	    str[i] = 'GOTONE';
	  }
	  //Exceptions
	  if(str[i] == "Womens") {
	  	str[i] = "Women\'s"; //This is YML compatiable for Drupal 8 Webform
	  }
	}
	return str.join(' ');
}
